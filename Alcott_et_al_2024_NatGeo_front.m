...
load('Relative_Iron_River_Dust.mat');
load('Relative_Iron_Glacial.Mat');
per.Dust_River_time = 1e6 * RelativeIron_River_Dust(:,1);
per.Dust_Mean = RelativeIron_River_Dust(:,2);
per.River_Min =RelativeIron_River_Dust(:,3);
per.River_Mean = RelativeIron_River_Dust(:,4);
per.River_Max = RelativeIron_River_Dust(:,5);
per.Glacial_time = 1e6 * RelativeIron_Glacial(:,1);
per.Glacial_Mean = RelativeIron_Glacial(:,2);
...
%% COPSE Carbon
starting.IC_Reservoir_0 = 3.193e18 ; 
starting.IC_Isotope  = 0 ; 
starting.GAST_0 = 288 ;

pars.y(22) = starting.IC_Reservoir_0 ;
pars.y(23) = starting.IC_Isotope ;
...
% Riverine P input.
pars.SRP_River_0 = 0.09e12 ; %Slomp and VC, 2007; Berner and Rao,1994
...
P_AuthP_P_0 = pars.SRP_River_0 + OP_P_Min_0 - SRP_P_D_0 - P_FeP_P_0 - P_PP_P_0 ; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        % sensparams.Degas_Rate  = 0.25 + rand*(2-0.25); 
        % 
        % sensparams.fbiota =  0.15 + rand*(1-0.15);
        % 
        % sensparams.Carbonate_Reservoir = 0.25 + rand*(3-0.25) ; 
        % 
...
randfixed  = 0.5 ;
        ...
        % parameter space to test
        sensparams.Degas_Rate  = 0.25 + randfixed*(2-0.25); 

        sensparams.fbiota =  0.15 + randfixed*(1-0.15);

        sensparams.Carbonate_Reservoir = 0.25 + randfixed*(3-0.25) ; 

        sensparams.CP = pars.CPoxic + randfixed*(1100-pars.CPoxic) ; 

        sensparams.EXP = 0.1 + randfixed* (0.75-0.1) ;

        sensparams.EXP2 = sensparams.EXP + randfixed* (1-sensparams.EXP) ;

        sensparams.EXPtiming = 1.7 + randfixed*(3-1.7);

        sensparams.PE = randfixed; 
