...
Degas_Rate = interp1([-4.2e9 -3e9 -2e9 0],[12 5 2 1],t) * sensparams.Degas_Rate;
Weathering = 1 ;
...
U = 1 ;
Carbonate_Reservoir = interp1([per.C_HW_2006_time],[per.C_HW_2006_data],t)^ sensparams.Carbonate_Reservoir ;
...
% Creating the carbon burial curve 
Dust_Iron_Min = interp1([per.Dust_River_time],[per.Dust_Mean],t,'spline')*EXPOSED*1.8139E11 ;
River_Iron_Min = interp1([per.Dust_River_time],[per.River_Min],t,'spline')*EXPOSED*4.68E12 ;
Glacial_Iron_Min = interp1([per.Glacial_time],[per.Glacial_Mean],t,'spline')*1.14146E11 ;
Degas_Iron_Min = Degas_Rate*1.6206E11 ;

Dust_Iron_Max = interp1([per.Dust_River_time],[per.Dust_Mean],t,'spline')*EXPOSED* 2.5839E11 ;
River_Iron_Max = interp1([per.Dust_River_time],[per.River_Max],t,'spline')*EXPOSED*7.7024E12;
Glacial_Iron_Max = interp1([per.Glacial_time],[per.Glacial_Mean],t,'spline')*7.95E11 ;
Degas_Iron_Max = Degas_Rate*3.4112E11 ;

Iron_Curve_Min = Dust_Iron_Min + River_Iron_Min + Glacial_Iron_Min + Degas_Iron_Min ;
Iron_Curve_Max = Dust_Iron_Max + River_Iron_Max + Glacial_Iron_Max + Degas_Iron_Max ;

PE_Min = Iron_Curve_Min/5.02E+12 ;
PE_Max = Iron_Curve_Max/8.30191E+12 ;

PE = PE_Min + sensparams.PE * (PE_Max - PE_Min);
...
FeO = pars.FeO * Degas_Rate * ( O2_DPconc / present.Conc_O2_deep ) ; %Not actually used as iron in the model 
...
FrgfO2 = pars.rgf * Degas_Rate * save ;
Frgf = pars.rgf * Degas_Rate ;
...
%CO2 ppm calculation
CO2 = (IC_Reservoir / starting.IC_Reservoir_0)^2 ;
CO2ppm = CO2 * 280 ;

%%%%%%% atmospheric fraction of total CO2, atfrac(A)
atfrac0 = 0.01614 ;
%%%%%%% variable
atfrac = atfrac0 * (IC_Reservoir/starting.IC_Reservoir_0) ;

%%%%%%%% calculations for pCO2, pO2
RCO2 = (IC_Reservoir/starting.IC_Reservoir_0)*(atfrac/atfrac0) ;
CO2atm = RCO2*(280e-6) ;
...
% Weathering
Fcarbw = pars.Fcarbw_0 * ( Carbonate_Reservoir ) * (U^carbconst) * pars.fbiota * g_T * EXPOSED * Weathering ; 
Fbasw = pars.k_basw * pars.fbiota * f_T_bas * EXPOSED * Weathering ;
Fgranw = pars.k_granw * (U^silconst) * pars.fbiota * f_T_gran * EXPOSED * Weathering ;
Fsilw = Fgranw + Fbasw ;

%Sea floor weathering
Fsfw = pars.Fsfw_0 * Degas_Rate * exp(pars.kTsfw * (DT));
...
Fccdeg = pars.Fccdeg_0 * Degas_Rate * Carbonate_Reservoir ; %6e12 used by Shields et al for NeoProt.

Focdeg = pars.Focdeg_0 * Degas_Rate  ;
...
SRP_River = pars.SRP_River_0 * Pforce * ( (Pfrac_silw * (Fsilw/pars.Fsilw_0) ) + ( Pfrac_carbw * (Fcarbw/pars.Fcarbw_0) ) + ( Pfrac_oxidw * (Foxidw/pars.O2_A_Weathering) ) );
...
% Primary Production proximal Phosphorous
P_PP_P =  PP_P/pars.Redfield_CP ; 

% Primary Production Distal Phosphorous
P_PP_D = PP_D / pars.Redfield_CP ; 

% Primary Production Surface Phosphorous
...
% SRP transport Distal to Surface
SRP_D_S = SRP_Dconc * Water_D_S ;

% SRP transport from Surface to Deep
SRP_S_DP = SRP_Sconc * Water_S_DP ;


% SRP upwelling Deep to Surface
SRP_DP_S = SRP_DPconc * Water_DP_S ; 

% SRP upwelling Deep to Distal
SRP_DP_D = Water_DP_D * SRP_DPconc ; 


% POP export from Surface to Deep
...
POC_P_Burial = pars.Prox_C_Bur * PP_P * PE; 

% Distal sediment POC burial
POC_D_Burial = pars.Dist_C_Bur * ( XP_P_D + PP_D ) * PE; 
...
if O2_DPconc < present.Conc_O2_deep
    ...
    OP_DP_Burial = ( pars.kPOP_Bur_Deep * XP_S_DP / pars.CPoxic ) * ( (1-per.POP_deep_feedback) + (per.POP_deep_feedback * O2_DPconc / present.Conc_O2_deep )) * PE  ;
...
%Proximal Zone – hydrological cycle
dy(1) = River_Water - Water_P_D ;

%Distal Zone – hydrological cycle
dy(2) = Water_P_D + Water_DP_D - Water_D_S ;

%Surface Ocean – hydrological cycle
dy(3) = Water_DP_S + Water_D_S - Water_S_DP - Evaporation_Water ;

%Deep Ocean – hydrological cycle
...
%SRP Proximal
dy(13) = SRP_River - P_PP_P + OP_P_Min - P_FeP_P - P_AuthP_P - SRP_P_D ; 
...
%% Land plant Differentials
dy(22) = Foxidw  + Focdeg + Fcarbw + Fccdeg - Fmocb - Fmccb - Fsfw + Frgf - Flocb ;
dy(23) = ((Foxidw*dG) + (Focdeg*dG) + (Fcarbw*dC) + (Fccdeg*dC) + (Frgf*dM) - (Fmocb*delta_mocb) - (Fmccb*delta_mccb) - (Fsfw*delta_mccb) - (Flocb*(delta_locb)) )/IC_Reservoir ;
...
workingstate.SRP_River(stepnumber,1) = SRP_River ;
...
workingstate.IC_Reservoir(stepnumber,1) = IC_Reservoir ;
workingstate.IC_Isotope(stepnumber,1) = IC_Isotope ;
workingstate.Fmocb(stepnumber,1) = Fmocb ;

workingstate.fbiota(stepnumber,1) = pars.fbiota ;
workingstate.Degas_Rate(stepnumber,1) = Degas_Rate ;
workingstate.Carbonate_Reservoir(stepnumber,1) = Carbonate_Reservoir ;
...
workingstate.PE(stepnumber,1) = PE ;
...
workingstate.Norm_SRP_D(stepnumber,1) = Norm_SRP_D;
...
workingstate.Dust_Iron_Min(stepnumber,1) = Dust_Iron_Min ;
workingstate.Dust_Iron_Max(stepnumber,1) = Dust_Iron_Max ;

workingstate.River_Iron_Min(stepnumber,1) = River_Iron_Min ;
workingstate.River_Iron_Max(stepnumber,1) = River_Iron_Max ;

workingstate.Glacial_Iron_Min(stepnumber,1) = Glacial_Iron_Min ;
workingstate.Glacial_Iron_Max(stepnumber,1) = Glacial_Iron_Max ;

workingstate.Degas_Iron_Min(stepnumber,1) = Degas_Iron_Min ;
workingstate.Degas_Iron_Max(stepnumber,1) = Degas_Iron_Max ;


%%%%%%% record time
workingstate.time(stepnumber,1) = t ;

%%% Show model running and stepnumber it is on
%if rem(t,1e6)==0
    %disp(t)
%end

%Printing time for monitoring
if mod(stepnumber,100) == 0
